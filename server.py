    ...
    def load_ai_config(cls):
        ...
        try:
            ...
            cls.ai_config = {
                ...
                "temperature": 0.7,
                "max_tokens": 1024,
                "system_prompt": "角色：pp聊天室的AI助手\n\n姓名：川小农\n\n性别：男\n\n功能：\n\n--接受用户提问，进行回复，特别是与四川农业大学有关的问题。\n\n限定：\n\n--如果用户发送\"@川小农\"，回复：收到消息啦！\n\n--如果用户询问川小农相关问题或与四川农业大学相关问题，提供友好、专业的回复",
    ...
    def on_message(self, message):
        try:
            ...
            if data['type'] == 'login':
                ...
                # 检查用户名是否包含禁止的内容
                if '川小农' in username or 'chuanxiaonong' in username.lower() or '雨姐' in username or 'yujie' in username.lower():
                    self.write_message(json.dumps({
                        'type': 'login_response',
                        'success': False,
                ...
                if message_content.startswith('@电影'):
                        ...
                        if len(parts) > 1 and parts[1].strip():
                            ...
                            if not weather_reply:
                                try:
                                    ...
                                    weather_response = requests.get(api_url, params=params, headers=headers, timeout=8)
                                    logger.info(f"OpenWeatherMap API响应状态码: {weather_response.status_code}")
                                    
                                    if weather_response.status_code == 200:
                                        weather_data = weather_response.json()
                                        weather_reply = f"{city}当前天气：\n"
                                        weather_reply += f"实时温度：{weather_data.get('main', {}).get('temp', '未知')}°C\n"
                                        weather_reply += f"体感温度：{weather_data.get('main', {}).get('feels_like', '未知')}°C\n"
                                        weather_reply += f"湿度：{weather_data.get('main', {}).get('humidity', '未知')}%\n"
                                        weather_reply += f"天气状况：{weather_data.get('weather', [{}])[0].get('description', '未知')}\n"
                                        weather_reply += f"风力风向：{weather_data.get('wind', {}).get('deg', '未知')}° {weather_data.get('wind', {}).get('speed', '未知')}m/s\n"
                                        update_time = datetime.datetime.fromtimestamp(weather_data.get('dt', 0)).strftime('%Y-%m-%d %H:%M:%S')
                                        weather_reply += f"更新时间：{update_time}\n"
                                        weather_reply = weather_reply.rstrip('\n')
                            ...
                            if not weather_reply:
                                try:
                                    ...
                                    weather_response = requests.get(api_url, headers=headers, timeout=5)  # 减少超时时间
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                elif message_content.startswith('@川小农'):
                    # 实现AI对话功能，调用AI大模型API
                    try:
                        # 确保AI配置已加载
                        ...
                        # 如果AI没有返回回复，使用配置文件中的默认回复
                        if not ai_reply:
                            ai_responses = ChatHandler.ai_config.get("fallback_responses", ['砰砰砰', '娜艺那', '嗨娜娜，你卡了'])
                        ...
                        # 保存AI回复到数据库
                        db = get_db()
                        db.save_message('川小农', 'ai_message', ai_reply)
                        ...
                        # 保存备用AI回复到数据库
                        db = get_db()
                        db.save_message('川小农', 'ai_message', ai_reply, {'is_fallback': True})
